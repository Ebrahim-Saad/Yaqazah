#!/usr/bin/env python3
"""Yaqazah: Location-aware Islamic prayer times and spiritual reminders for the Omarchy shell."""
...
STATE_DIR = Path(os.environ.get("XDG_STATE_HOME") or Path.home() / ".local/state") / "yaqazah"
CACHE_DIR = Path(os.environ.get("XDG_CACHE_HOME") or Path.home() / ".cache") / "yaqazah"
...
THEMED_ICON = CACHE_DIR / "yaqazah.svg"
...
class YaqazahError(RuntimeError):
...
def read_limited(stream: Any, max_bytes: int) -> bytes:
    ...
    while True:
        ...
        total += len(chunk)
        if total > max_bytes:
            raise YaqazahError("Input exceeded the allowed size.")
        chunks.append(chunk)
...
def open_directory(path: Path) -> int:
    ...
    try:
        ...
        if not stat.S_ISDIR(metadata.st_mode) or metadata.st_uid != os.getuid():
            raise YaqazahError("A local data directory is not a safe user-owned directory.")
        return directory_fd
    except Exception:
        os.close(directory_fd)
        raise
...
def read_text_limited(path: Path, max_bytes: int) -> str:
    ...
    try:
        ...
        if not stat.S_ISREG(metadata.st_mode) or metadata.st_uid != os.getuid():
            raise YaqazahError("A local data file is not a safe user-owned regular file.")
...
def atomic_write_text(path: Path, value: str, max_bytes: int) -> None:
    ...
    if len(payload) > max_bytes:
        raise YaqazahError("Output exceeded the allowed size.")
...
def validate_json_shape(value: Any) -> None:
    ...
    def visit(item: Any, depth: int) -> None:
        ...
        if item_count > JSON_MAX_ITEMS or depth > JSON_MAX_DEPTH:
            raise YaqazahError("JSON data exceeded the allowed complexity.")
        if isinstance(item, str):
            if len(item) > JSON_MAX_STRING_LENGTH:
                raise YaqazahError("JSON data contained an oversized string.")
        elif isinstance(item, dict):
            if len(item) > JSON_MAX_CONTAINER_ITEMS:
                raise YaqazahError("JSON data contained too many fields.")
            for key, child in item.items():
                if not isinstance(key, str) or len(key) > 64:
                    raise YaqazahError("JSON data contained an invalid field name.")
                visit(child, depth + 1)
        elif isinstance(item, list):
            if len(item) > JSON_MAX_CONTAINER_ITEMS:
                raise YaqazahError("JSON data contained too many entries.")
            for child in item:
                visit(child, depth + 1)
        elif item is not None and not isinstance(item, (bool, int, float)):
            raise YaqazahError("JSON data contained an unsupported value.")
...
def read_json(path: Path) -> dict[str, Any] | None:
    ...
    except (OSError, UnicodeError, ValueError, YaqazahError):
        return None
...
def request_json(url: str, timeout: float = 8) -> dict[str, Any]:
    ...
    try:
        ...
        raise YaqazahError(f"Network request failed: {error}") from error
    if not isinstance(payload, dict):
        raise YaqazahError("The location or prayer service returned invalid data.")
    return payload
...
def provider_text(
    ...
    if value is None or value == "":
        if required:
            raise YaqazahError(f"The remote service did not include {field}.")
        return ""
    if not isinstance(value, (str, int)) or isinstance(value, bool):
        raise YaqazahError(f"The remote service returned an invalid {field}.")
...
def normalize_location(raw: dict[str, Any], source: str) -> dict[str, Any]:
    if not isinstance(raw, dict):
        raise YaqazahError("The location service returned invalid data.")
    ...
    try:
        ...
        raise YaqazahError("The detected location did not include usable coordinates.") from error
    if not -90 <= latitude <= 90 or not -180 <= longitude <= 180:
        raise YaqazahError("The detected location included invalid coordinates.")
    return {
        "city": city,
        "country": country,
...
def detect_location() -> dict[str, Any]:
    ...
    for url, parser in providers:
        ...
        except YaqazahError as error:
            errors.append(str(error))
    raise YaqazahError("Automatic city detection failed. Set a manual city in the plugin settings.")


def geocode_location(city: str, country: str) -> dict[str, Any]:
    query = ", ".join(part for part in (city.strip(), country.strip()) if part)
    if not city.strip():
        raise YaqazahError("Manual location needs a city in the plugin settings.")
    ...
    if not isinstance(results, list) or not results or len(results) > 8:
        raise YaqazahError(f"No location was found for {query}.")
    if not all(isinstance(result, dict) for result in results):
        raise YaqazahError("The location service returned invalid results.")
...
def search_cities(query: str, count: int = 5) -> list[dict[str, Any]]:
    ...
    if not clean_query:
        return []
    params = urllib.parse.urlencode({"name": clean_query, "count": max(1, min(count, 10)), "language": "en", "format": "json"})
    payload = request_json(f"https://geocoding-api.open-meteo.com/v1/search?{params}")
    results = payload.get("results")
...
def parse_api_time(value: Any) -> str:
    ...
    if not match:
        raise YaqazahError("The prayer service returned an invalid time.")
...
def fetch_timings(
    ...
    if payload.get("code") != 200 or not isinstance(payload.get("data"), dict):
        raise YaqazahError("The prayer service could not calculate times for this location.")
    data = payload["data"]
    raw_timings = data.get("timings")
    if not isinstance(raw_timings, dict):
        raise YaqazahError("The prayer service returned invalid timings.")
...
def theme_color() -> str:
    ...
    except (OSError, UnicodeError, YaqazahError):
        return "#c9c7cd"
...
def render_themed_icon() -> Path:
    ensure_dirs()
    try:
        source = read_text_limited(PLUGIN_DIR / "assets/yaqazah.svg", SVG_MAX_BYTES)
    except (OSError, UnicodeError, YaqazahError) as error:
        raise YaqazahError("The Yaqazah icon could not be read safely.") from error
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Yaqazah prayer and reminder service")
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        if args.search_city is not None:
            results = search_cities(args.search_city)
            print(json.dumps({"ok": True, "results": results}, ensure_ascii=True, separators=(",", ":")))
            return 0
        ...
        print(json.dumps({"ok": False, "error": f"Yaqazah failed: {error}"}, ensure_ascii=True))
        return 1
